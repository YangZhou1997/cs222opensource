...
from sketchlib.sketch_coding_maxsketch import sketch_transform
...
parser.add_argument('--model', '-a', metavar='MODEL', default='resnet_quantized_float_bn_pruning',
                    choices=model_names,
...
parser.add_argument('--model_config', default="{'depth': 18}",
...
parser.add_argument('--resume', default='./results/quantized_resnet18_pruning/model_best.pth.tar', 
...
def sketch_layer(model, index):
    ...
    wei.data=torch.Tensor(sketch_transform(quan_wei.data.cpu().numpy().reshape((a,b*c*d)), 8, 0.8, float_step).reshape((a,b,c,d)))
