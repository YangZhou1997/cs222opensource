...
from models.modules.quantize import quantize
from models.modules.quantize import QConv2d_P as QConv2d
from models.modules.quantize import QLinear_P as QLinear
from torchvision import datasets, transforms

from sketchlib.sketch_coding_maxsketch_pruning import sketch_transform
...
parser.add_argument('--results_dir', metavar='RESULTS_DIR', default='./mnist_results',
                    help='results dir')
parser.add_argument('--save', metavar='SAVE', default='',
                    help='saved folder')
parser.add_argument('--dataset', metavar='DATASET', default='mnist',
                    help='dataset name or folder')
parser.add_argument('--model', '-a', metavar='MODEL', default='lenet_quantized_float_bn_pruning',
                    choices=model_names,
...
parser.add_argument('--resume', default='/home/simonx/Documents/cs222projectcode/mnist_results/pruning_baseline/model_best.pth.tar', 
...
def weight_prune(model, pruning_perc):
    ...
    for layer in get_layer_list(model):
        ...
        all_weights = layer.weight.cpu().data.abs().numpy().flatten()
        threshold = np.percentile(np.array(all_weights), pruning_perc)
        pruned_inds = layer.weight.data.abs() > threshold
...
def sketch_layer(model, index, compre_ratio=0.8):
    ...
    logging.info('Magnitude of weights: ' + str(quan_wei.data.abs().mean().item()))
    wei.data=torch.Tensor(sketch_transform(quan_wei.data.cpu().numpy().reshape((a,b*c*d)), 8, compre_ratio, float_step).reshape((a,b,c,d)))
    logging.info('Magnitude of distance: '+ str(torch.abs(wei.data-quan_wei.data.cpu()).mean().item()))
    logging.info('STD of distance: ' + str(torch.abs(wei.data-quan_wei.data.cpu()).std().item()))
    wei.cuda()
