...
from models.modules.quantize import QConv2d, QLinear, quantize
from sketchlib.sketch_coding_maxsketch_double import sketch_transform
...
def sketch_layer(model, index):
    ...
    wei.data=torch.Tensor(sketch_transform(quan_wei.data.cpu().numpy().reshape((a,b*c*d)), 8, 0.9, float_step).reshape((a,b,c,d)))
    wei.cuda()
